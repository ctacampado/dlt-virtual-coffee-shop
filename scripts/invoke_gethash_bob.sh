...
export GETHASH=$(echo -n "{\"Key\":\"12351\"}" | base64 | tr -d \\n)
peer chaincode invoke -C channela -n coffeehubcc --peerAddresses $CORE_PEER_ADDRESS --peerAddresses $OTHER_PEER_ADDRESS --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE --tlsRootCertFiles $OTHER_PEER_TLS_ROOTCERT_FILE -c '{"Args":["getHash"]}' --transient "{\"hashReq\":\"$GETHASH\"}" --tls --cafile $ORDERER_CA